...
let gameState = {
    ...
    subjectStats: {}, // { "Assunto": { total: 0, correct: 0, incorrect: 0 } }
...
app.prepare().then(() => {
    ...
    function getAnswersCount() {
        ...
        return all.filter(p => p.selectedOption !== null).length;
    }

    function checkAllPlayersAnswered() {
        if (gameState.status !== 'PLAYING') return;

        // Get all currently connected players in the game array (ignoring the disconnected ones stored separately)
        const connectedPlayers = gameState.players;

        // Find players who are expected to answer (excluding late joiners)
        const expectedParticipants = connectedPlayers.filter(p => !p.joinedLate);

        // If no one is expected to answer (e.g. only late joiners are in the room), don't auto-resolve
        if (expectedParticipants.length === 0) return;

        // Count how many of those expected participants have actually provided an answer
        const answeredCount = expectedParticipants.filter(p => p.selectedOption !== null).length;

        console.log(`[Game] Checking Auto-Resolve: ${answeredCount}/${expectedParticipants.length} expected players have answered.`);

        if (answeredCount >= expectedParticipants.length) {
            console.log(`[Game] All expected participants answered. Showing results.`);
            showResults();
        }
    }
    ...
    function sendPlayerCurrentState(socket, player) {
        ...
        socket.emit('gameStateUpdate', {
            ...
            joinedLate: player && player.score === 0 && player.totalTime === 0 && player.answeredCurrent && gameState.status === 'PLAYING',
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('joinRoom', ({ pin, name, playerId }) => {
            ...
            if (trimmedPin === gameState.pin) {
                ...
                // Allowed states: LOBBY, PLAYING, RESULTS
                if (gameState.status === 'FINAL_RANKING') {
                    return socket.emit('error', 'O jogo já foi encerrado.');
                }
                ...
                const checkAnsweredState = gameState.status === 'PLAYING';
                ...
                const newPlayer = {
                    ...
                    totalTime: 0,
                    correctCount: 0,
                    answeredCurrent: checkAnsweredState, // If joining mid-question, mark as answered so they wait
                    joinedLate: checkAnsweredState,
                    selectedOption: null,
                    lastFeedback: null
                };
                ...
                if (checkAnsweredState) {
                    io.emit('answersUpdate', { count: getAnswersCount() });
                    checkAllPlayersAnswered();
                }

            } else {
                console.log(`[Join Error] Failed to join Room! Received PIN: "${pin}", Game PIN: "${gameState.pin}", Status: "${gameState.status}"`);
        ...
        socket.on('submitAnswer', ({ optionIndex }) => {
            ...
            if (player && gameState.status === 'PLAYING' && !player.answeredCurrent) {
                ...
                if (!gameState.subjectStats[subject]) {
                    gameState.subjectStats[subject] = { total: 0, correct: 0, incorrect: 0 };
                }
                gameState.subjectStats[subject].total += 1;
                ...
                if (optionIndex === question.correctIndex) {
                    ...
                    gameState.subjectStats[subject].correct += 1;
                } else {
                    player.lastFeedback = { correct: false, points: 0, totalScore: player.score };
                    gameState.subjectStats[subject].incorrect += 1;
                }
    ...
    function showResults() {
        ...
        // Auto-end the game after the last question
        if (isLastQuestion) {
            setTimeout(() => {
                if (gameState.status === 'RESULTS') {
                    console.log('[Game] Last question answered. Auto-ending game.');
                    showFinalRanking();
                }
            }, 5000);
        }
    ...
    function showFinalRanking() {
        ...
        // Send each connected player their individual position
        gameState.players.forEach(player => {
            const position = ranking.findIndex(p => p.playerId === player.playerId) + 1;
        ...
        // Also broadcast to host (sockets not in gameState.players, e.g. the host)
        io.emit('finalRankingHost', { ranking: ranking, subjectStats: gameState.subjectStats });
    }
