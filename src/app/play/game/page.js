...
export default function PlayerGame() {
    ...
    const [selectedAnswer, setSelectedAnswer] = useState(() => {
        ...
        try {
            ...
            return parsed && typeof parsed === 'object' ? parsed.option : null;
        } catch (e) {
            return null;
        }
    });
        ...
        if (socket) {
                    ...
                    if (data.status === 'PLAYING') {
                        if (data.joinedLate) {
                            ...
                            if (currentStatus !== 'FEEDBACK' && currentStatus !== 'TIMEOUT') {
                                // Prefer server option, otherwise try to extract from sessionStorage
                                let option = data.selectedOption;
                                if (option === null) {
                                    try {
                                        const saved = JSON.parse(sessionStorage.getItem('selectedAnswer'));
                                        option = saved && typeof saved === 'object' ? saved.option : null;
                                    } catch (e) { option = null; }
                                }
                                setSelectedAnswer(option !== null ? option : 0); // fallback to 0 instead of NaN
                                setStatus('SUBMITTED');
                            }
                        } else {
                            // Recovery from sessionStorage if server didn't catch it yet
                            // Only recover if the saved answer matches the CURRENT question number.
                            let validSavedOption = null;
                            try {
                                const saved = sessionStorage.getItem('selectedAnswer');
                                if (saved) {
                                    const parsed = JSON.parse(saved);
                                    if (parsed && typeof parsed === 'object' && parsed.qNumber === data.currentQuestionNumber) {
                                        validSavedOption = parsed.option;
                                    }
                                }
                            } catch (e) {}

                            if (validSavedOption !== null) {
                                setSelectedAnswer(validSavedOption);
                                setStatus('SUBMITTED');
                                // Re-submit to the server since server says hasn't answered
                                socket.emit('submitAnswer', { optionIndex: validSavedOption });
                            } else if (currentStatus !== 'SUBMITTED' && currentStatus !== 'FEEDBACK' && currentStatus !== 'TIMEOUT' && currentStatus !== 'WAITING_NEXT') {
                                // Clear old stale answers
                                sessionStorage.removeItem('selectedAnswer');
                                setStatus('ANSWERING');
                            }
                        }
            ...
            socket.on('joinedSuccess', (data) => {
                if (data.isReconnection) {
                    console.log('Reconexão bem-sucedida! Aguardando gameStateUpdate para validar respostas...');
                    // We no longer automatically submit here because we don't know the current question number yet.
                    // This prevents sending stale answers from previous questions. 
                    // Submission is now handled safely within 'gameStateUpdate'.
                }
            });
    ...
    const handleAnswer = (index) => {
        if (status === 'ANSWERING' && socket) {
            ...
            setStatus('SUBMITTED');
            sessionStorage.setItem('selectedAnswer', JSON.stringify({ 
                option: index, 
                qNumber: currentQuestionNumber 
            }));
        }
    };
