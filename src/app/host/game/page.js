...
export default function HostGame() {
    ...
    const downloadReport = () => {
        if (!subjectStats) return alert('Nenhum dado de assunto disponível.');

        const rows = [];
        for (const [subject, stats] of Object.entries(subjectStats)) {
            const correctPercent = stats.total > 0 ? ((stats.correct / stats.total) * 100).toFixed(1) : '0.0';
            const incorrectPercent = stats.total > 0 ? ((stats.incorrect / stats.total) * 100).toFixed(1) : '0.0';
            ...
            rows.push({
                ...
                '% Acertos': `${correctPercent}%`,
                '% Erros': `${incorrectPercent}%`
            });
        }

        const ws = XLSX.utils.json_to_sheet(rows);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Desempenho por Assunto');
        XLSX.writeFile(wb, 'relatorio_assuntos.xlsx');
    };
                ...
                <div className="arena-card text-slate-800 w-full max-w-2xl bg-white/95 backdrop-blur-md flex flex-col max-h-[50vh]">
                    ...
                    <div className="mt-8 flex flex-col gap-3 shrink-0">
                        {subjectStats && Object.keys(subjectStats).length > 0 && (
                            <button onClick={downloadReport} className="bg-emerald-600 hover:bg-emerald-500 text-white w-full py-4 text-xl font-black uppercase rounded-2xl shadow-xl transition-all">
                                📊 Baixar Relatório por Assuntos
                            </button>
                        )}
                        <button onClick={() => router.push('/host/dashboard')} className="arena-button btn-primary w-full text-xl py-4 uppercase">Voltar ao Início</button>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="min-h-screen bg-[#dcfce7] flex flex-col font-sans">
            ...
            <div className="p-6 md:p-10 text-center bg-white shadow-lg min-h-[120px] md:min-h-[180px] flex flex-col items-center justify-center border-b-8 border-[var(--arena-primary)] relative">
                ...
                {/* Prominent QR Code and PIN for late joiners */}
                {joinUrl && (
                    <div className="absolute top-4 left-4 md:left-8 flex items-center gap-3 md:gap-5 bg-slate-50 p-3 rounded-xl border border-slate-200 shadow-sm origin-top-left transition-transform hover:scale-105 z-20">
                        <div className="w-24 h-24 md:w-32 md:h-32 bg-white p-2 rounded-lg border border-slate-100 shrink-0">
                            <QRCode
                                value={joinUrl}
                                size={128}
                        ...
                        <div className="flex flex-col text-left">
                            <span className="text-xs md:text-sm font-black uppercase text-slate-400 tracking-widest leading-none mb-1 md:mb-2">Entrar na Arena</span>
                            <span className="text-3xl md:text-5xl font-black text-slate-800 tracking-tighter leading-none">{pin}</span>
                        </div>
                    </div>
                )}
                ...
                <h1 className="text-3xl md:text-5xl font-black text-slate-800 leading-tight drop-shadow-sm px-6 md:px-[300px] lg:px-[350px] mt-40 md:mt-0 relative z-10 w-full">
                    {question.question}
                </h1>
            </div>
